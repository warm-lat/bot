...
class Settings(BaseModel):
    ...
    async def dispatch_log(
        ...
        **kwargs,
        ...
        #         "module": module,
        #         "punishment": getattr(self, module).punishment,
        #         "perpetrator_id": str(perpetrator.id),
        #         "perpetrator_roles": [str(r.id) for r in perpetrator.roles],
        #         "perpetrator_joined_at": perpetrator.joined_at.isoformat() if perpetrator.joined_at else None,
        #         "threshold": getattr(self, module).threshold,
        #         "duration": getattr(self, module).duration,
        #         "elapsed_time": elapsed,
        #         "failure": failure,
        #         "details": details,
        #         "description": description,
        #         "bot_permissions": [p[0] for p in bot_member.guild_permissions if p[1]],
        #         "timestamp": current_time.isoformat()
        #     }

            # posthog_cloud.capture(
            #     str(bot.user.id),
            #     event="antinuke_protection",
            #     properties=properties,
            #     groups={"guild": str(self.guild.id)}
            # )
            
            # if posthog_selfhost:
            #     posthog_selfhost.capture(
            #         str(bot.user.id),
            #         event="antinuke_protection", 
            #         properties=properties,
            #         groups={"guild": str(self.guild.id)}
        ...
        #     log.warning(f"Analytics failed for antinuke_{module} - Will retry in 5 minutes")
