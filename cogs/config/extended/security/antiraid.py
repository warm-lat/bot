...
class AntiRaid(MixinMeta, metaclass=CompositeMetaClass):
    ...
    async def submit_incident(
        ...
        try:
            ...
            #         event="antiraid_incident",
            #         properties=properties,
            #         groups={"guild": str(guild.id)}
            #     )
    ...
    async def notify_perpetrator(
        ...
        #         "member_created_at": member.created_at.isoformat(),
        #         "member_joined_at": member.joined_at.isoformat() if member.joined_at else None,
        #         "member_avatar": bool(member.avatar),
        #         "member_bot": member.bot,
        #         "member_flags": [str(flag.name) for flag in member.public_flags],
        #         "timestamp": current_time.isoformat()
        #     }
        ...
        # except Exception as e:
        #     self.error_cooldown[error_key] = current_time
        #     log.warning(f"Analytics failed for antiraid action - Will retry in 5 minutes")
