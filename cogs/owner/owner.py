...
class DocketView(discord.ui.View):
    ...
    @discord.ui.select(
        placeholder="Select a docket to view details...",
        min_values=1,
        max_values=1,
        custom_id="docket_selector"
    )
    async def select_docket(
        self, 
        interaction: discord.Interaction, 
        select: discord.ui.Select
    ):
        docket_id = int(select.values[0])
        ...
        embed = Embed(
            ...
            description=(
                ...
                f"**Created:** {discord.utils.format_dt(docket['created_at'])}\n"
                f"**Last Updated:** {discord.utils.format_dt(docket['last_updated'])}\n\n"
                f"**GPT Summary:**\n{docket['gpt_summary']}\n\n"
    ...
    async def update_options(self):
        """Update the select menu options with latest dockets"""
        ...
        self.select_docket.options = [
            discord.SelectOption(
                label=f"{d['title'][:80]}...",
                value=str(d['id']),
                description=f"Status: {d['status']} • Created: {d['created_at'].strftime('%d/%m/%Y %H:%M UTC')}",
                emoji="📋"
            ) for d in dockets
        ]
...
class Owner(
    ...
    async def create_docket_status_embed(self) -> discord.Embed:
        """Create the docket status overview embed"""
        ...
        embed = Embed(
            title=f"{config.EMOJIS.DOCKET.INFO} Docket Overview",
            description=(
                "Use the dropdown below to view detailed information about each docket.\n\n"
                "**Status Guide:**\n"
        ...
        if active_dockets:
            ...
            for docket in active_dockets:
                ...
                active_list.append(
                    f"{emoji} **{docket['title'][:50]}**... "
                    f"(Created: {discord.utils.format_dt(docket['created_at'])})"
                )
            
            embed.add_field(
                name="Active Dockets",
                value="\n".join(active_list) if active_list else "No active dockets",
                inline=False
            )
            ...
            for status, count in status_counts.items():
                emoji = status_emojis.get(status, f"{config.EMOJIS.DOCKET.CYAN}")
                stats.append(f"{emoji} **{status.title()}:** {count}")
            
            embed.add_field(
                name=f"Statistics (Total: {total})",
                value="\n".join(stats),
                inline=False
            )
        ...
        embed.set_footer(text="Updates every 30 minutes • Last updated")
        return embed

    async def update_docket_channels(self):
        """Update all docket status channels"""
    ...
    async def cog_load(self):
        self.docket_update_task.start()

    async def cog_unload(self):
        self.docket_update_task.cancel()
    ...
    @docket_update_task.before_loop
    async def before_docket_update(self):
