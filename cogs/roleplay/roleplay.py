...
class Roleplay(Cog):
    ...
    async def threesome_send(self, ctx: Context, member: Optional[Member], member2: Optional[Member], category: str) -> Message:
        """
        Requests the API,
        proxies the request,
        and structures the embed.
        ...
        async with ctx.typing():
            ...
            async with ClientSession(connector=connector) as session:
                async with session.get(url, proxy=config.CLIENT.WARP) as response:
                    ...
                    embed.description = (
                            f"{ctx.author.mention} {ACTIONS[category]} {member.mention} and {member2.mention}"
                        )
