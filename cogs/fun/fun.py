...
class Fun(Cog):
    ...
    async def domain_setup(self, ctx: Context, domain: str):
        ...
        if len(verified_domains) >= 5:
            return await ctx.warn("You can only have 5 verified domains at once!")
        ...
        embed = Embed(
            description=(
                ...
                f"`_warmlat-verify.{domain}` → `\"warmlat-verifyuser={ctx.author.name}\"`\n\n"
                "**Important Notes:**\n"
    ...
    async def domain_verify(self, ctx: Context, domain: str):
        ...
        try:
            ...
            txt_records = await self.bot.loop.run_in_executor(
                None, 
                lambda: dns.resolver.resolve(f'_warm-verify.{domain}', 'TXT')
            )
            txt_valid = any(f"warm-verifyuser={ctx.author.name}" in str(record) for record in txt_records)
