...
class Paginator(View):
    ...
    def add_buttons(self):
        for button in (
            Button(
                ...
                emoji=config.EMOJIS.CONTEXT.LEFT or "⬅",
            ),
            Button(
            ...
            Button(
                custom_id="next",
                style=ButtonStyle.secondary,
                emoji=config.EMOJIS.CONTEXT.RIGHT or "➡",
            ),
            Button(
                custom_id="cancel",
                style=ButtonStyle.secondary,
                emoji=config.EMOJIS.CONTEXT.DENY or "⏹",
            ),
        ):
        ...
        if not embed:
            if isinstance(entries[0], str):
                ...
                for index, entry in enumerate(entries):
                    if "page" not in entry and counter and not hide_index:
    ...
    async def start(self, content: str = None) -> Message:
        ...
        if len(self.entries) == 1:
            if content:
                self.message = await self.ctx.send(content=content)
            else:
                self.message = (
            ...
            if content:
                self.message = await self.ctx.send(content=content, embed=page, view=self)
            else:
                self.message = (
                    await self.ctx.send(embed=page, view=self)
                    if isinstance(page, Embed)
                    else await self.ctx.send(content=page, view=self)
                )
